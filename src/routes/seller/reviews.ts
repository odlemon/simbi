...
router.get("/", authenticateSellerOrStaff, sellerReviewController.getReviews.bind(sellerReviewController));
...
// Note: This must come before /:id to avoid route conflicts
router.get("/inventory/:inventoryId/rating", authenticateSellerOrStaff, sellerReviewController.getRatingStats.bind(sellerReviewController));

// POST /api/seller/reviews/:id/respond - Create or update response to a review
router.post("/:id/respond", authenticateSellerOrStaff, sellerReviewController.respondToReview.bind(sellerReviewController));

// PUT /api/seller/reviews/:id/respond - Update existing response to a review
router.put("/:id/respond", authenticateSellerOrStaff, sellerReviewController.updateResponse.bind(sellerReviewController));

// GET /api/seller/reviews/:id - Get a specific review by ID (must be last to avoid conflicts)
router.get("/:id", authenticateSellerOrStaff, sellerReviewController.getReviewById.bind(sellerReviewController));
